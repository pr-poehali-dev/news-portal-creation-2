...
const PressReleaseDetail = () => {
  const { id } = useParams();
  const [pressRelease, setPressRelease] = useState<PressReleaseDetail | null>(null);
  ...
  const loadPressRelease = async () => {
    try {
      const response = await fetch(`${API_URL}?resource=press-releases&id=${id}`);
        ...
        <main className="container mx-auto px-4 py-8">
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <h1 className="text-2xl font-bold mb-2">Пресс-релиз не найден</h1>
        ...
        <article className="bg-white rounded-lg shadow-lg overflow-hidden">
          ...
          <div className="p-8 max-w-4xl mx-auto">
            <div className="flex items-center gap-3 mb-4 flex-wrap">
              <Badge variant="default">{pressRelease.category_label}</Badge>
              <span className="text-sm text-muted-foreground flex items-center gap-1">
                <Icon name="Clock" size={14} />
                {pressRelease.time_label}
              </span>
              {pressRelease.views && (
                <span className="text-sm text-muted-foreground flex items-center gap-1">
                  <Icon name="Eye" size={14} />
                  {pressRelease.views} просмотров
                </span>
              )}
            </div>
            ...
            {pressRelease.author && (
              <div className="flex items-center gap-2 mb-6 text-muted-foreground">
                ...
                <span className="text-sm">Автор: {pressRelease.author}</span>
              </div>
            )}
            ...
            {pressRelease.video_url && (
              <div className="mb-8">
                <div className="aspect-video bg-muted rounded-lg flex items-center justify-center">
                  <a 
                    href={pressRelease.video_url} 
                    target="_blank" 
            ...
            {pressRelease.podcast_url && (
              <div className="mb-8">
                <div className="bg-muted rounded-lg p-6 flex items-center justify-center">
                  <a 
                    href={pressRelease.podcast_url} 
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 text-primary hover:underline"
                  >
            ...
            {pressRelease.images && pressRelease.images.length > 0 && (
              <div className="mb-8 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {pressRelease.images.map((img, index) => (
            ...
            {pressRelease.links && pressRelease.links.length > 0 && (
              <div className="mb-8">
                <h2 className="text-2xl font-bold mb-4">Полезные ссылки</h2>
                <div className="space-y-2">
                  {pressRelease.links.map((link, index) => (
                    <a 
                      key={index}
                      href={link.url}
                      target="_blank"
            ...
            {pressRelease.source_url && (
              <div className="border-t pt-6 text-sm text-muted-foreground">
                <a 
                  href={pressRelease.source_url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center gap-2 hover:text-primary"
                >
                  <Icon name="Link" size={16} />
                  Источник: {pressRelease.source_url}
                </a>
              </div>
            )}
          </div>
