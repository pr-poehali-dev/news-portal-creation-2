...
const AdminPressReleasesEdit = () => {
  ...
  const [form, setForm] = useState<PressReleaseForm>({
    title: '',
    text: '',
    shortDescription: '',
    videoUrl: '',
  ...
  const loadPressRelease = async () => {
    try {
      ...
      toast.error('Ошибка загрузки пресс-релиза');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = isNew 
        ? `${API_URL}?resource=press-releases`
        : `${API_URL}?resource=press-releases&id=${id}`;
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          category_code: 'press-releases',
          time_label: 'Только что'
        })
      });

      if (response.ok) {
        toast.success(isNew ? 'Пресс-релиз создан' : 'Пресс-релиз обновлен');
      ...
      <div className="flex-1">
        <header className="bg-white border-b px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Button variant="ghost" onClick={() => navigate('/admin/press-releases')}>
                <Icon name="ArrowLeft" size={18} className="mr-2" />
                Назад
              </Button>
              <h1 className="text-2xl font-bold">
                {isNew ? 'Добавить пресс-релиз' : 'Редактировать пресс-релиз'}
              </h1>
            </div>
        ...
        <main className="p-8">
          ...
          <div className="mt-6 flex gap-3 justify-end">
            ...
            <Button variant="outline" onClick={() => navigate('/admin/press-releases')}>
              Отменить
            </Button>
            <Button onClick={handleSave} className="bg-green-600 hover:bg-green-700">
              <Icon name="Save" size={18} className="mr-2" />
...
export default AdminPressReleasesEdit;
