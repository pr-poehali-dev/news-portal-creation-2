...
const AdminHoroscopes = () => {
  ...
  const loadHoroscopes = async () => {
    try {
      const response = await fetch(`${API_URL}?resource=horoscopes`);
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Удалить этот гороскоп?')) return;
    try {
      ...
      toast.error('Ошибка удаления гороскопа');
    }
  };

  const handleEdit = (id: number) => {
    navigate(`/admin/horoscopes/edit/${id}`);
  };

  const filteredHoroscopes = horoscopes.filter(item => 
      ...
      <div className="flex-1">
        <header className="bg-white border-b px-8 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold">Гороскопы</h1>
            <Button onClick={() => navigate('/admin/horoscopes/edit/new')}>
