...
const AdminHoroscopesEdit = () => {
  ...
  const [form, setForm] = useState<HoroscopeForm>({
    title: '',
    text: '',
    shortDescription: '',
    videoUrl: '',
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = isNew 
        ? `${API_URL}?resource=horoscopes`
        : `${API_URL}?resource=horoscopes&id=${id}`;
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          category_code: 'horoscopes',
          time_label: 'Только что'
        })
      });

      if (response.ok) {
        toast.success(isNew ? 'Гороскоп создан' : 'Гороскоп обновлен');
      ...
      <div className="flex-1">
        <header className="bg-white border-b px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Button variant="ghost" onClick={() => navigate('/admin/horoscopes')}>
                <Icon name="ArrowLeft" size={18} className="mr-2" />
                Назад
              </Button>
              <h1 className="text-2xl font-bold">
                {isNew ? 'Добавить гороскоп' : 'Редактировать гороскоп'}
              </h1>
            </div>
        ...
        <main className="p-8">
          ...
          <div className="mt-6 flex gap-3 justify-end">
            ...
            <Button variant="outline" onClick={() => navigate('/admin/horoscopes')}>
              Отменить
            </Button>
            <Button onClick={handleSave} className="bg-green-600 hover:bg-green-700">
              <Icon name="Save" size={18} className="mr-2" />
...
export default AdminHoroscopesEdit;