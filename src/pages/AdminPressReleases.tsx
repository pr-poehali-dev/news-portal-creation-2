...
const AdminPressReleases = () => {
  ...
  const loadPressReleases = async () => {
    try {
      const response = await fetch(`${API_URL}?resource=press-releases`);
      const data = await response.json();
      setPressReleases(data);
    } catch (error) {
      console.error('Error loading press releases:', error);
      toast.error('Ошибка загрузки пресс-релизов');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Удалить этот пресс-релиз?')) return;
    try {
      const response = await fetch(`${API_URL}?resource=press-releases&id=${id}`, {
        method: 'DELETE'
      });
      if (response.ok) {
        toast.success('Пресс-релиз удален');
        loadPressReleases();
      }
    } catch (error) {
      console.error('Error deleting press release:', error);
      toast.error('Ошибка удаления пресс-релиза');
    }
  };
      ...
      <div className="flex-1">
        <header className="bg-white border-b px-8 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold">Пресс-релизы</h1>
            <Button onClick={() => navigate('/admin/press-releases/edit/new')}>
